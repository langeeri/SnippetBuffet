...
This module provides functionality to perform a port scan on one or multiple target IP addresses. It includes capabilities to scan a range of ports on each target to identify open ports.

The script allows for scanning multiple targets by accepting a comma-separated list of IP addresses. The user can specify the range of ports to scan for each target.
...
Usage:
Run the script and enter the targets and the number of ports to scan when prompted. The script will display the status of each port (open or closed) for each target IP address.

Note:
- The script requires the 'socket' and 'termcolor' modules.
- Port scanning can be illegal or considered hostile activity on certain networks. Ensure you have authorization before scanning ports on any network.
